...
GLUSboolean initGame(GLUSvoid)
{
	...
	SkySP sky = SkySP(new SkySphere(500.0f, 500.0f, 500.0f, "uffizi_cross.dds"));

	SkyManager::getInstance()->setSky("MySky", sky);

	SkyManager::getInstance()->setActiveSky("MySky");
	...
	entity = entityFactory.loadFbxFile("Seagull", filename, 1.0f);
	if (!entity.get())
	{
	...
	entity->setPosition(position);
	entity->setRotation(0.0f, 180.0f, 0.0f);
	entity->setAnimation(0, 0);
	...
	filename = "goober.fbx";
	entity = entityFactory.loadFbxFile("Goober", filename, 1.0f);
	if (!entity.get())
	{
		glusLogPrint(GLUS_LOG_ERROR, "File not found %s", filename.c_str());
	...
	// Light up the material a little bit - just this one
	...
	filename = "goober@idle.fbx";
	entity = entityFactory.loadFbxFile("GooberIdle", filename, 1.0f);
	if (!entity.get())
	{
		glusLogPrint(GLUS_LOG_ERROR, "File not found %s", filename.c_str());
	...
	position = Point4(-40.0f, 10.0f, -16.0f);

	filename = "goober@jump.fbx";
	entity = entityFactory.loadFbxFile("GooberJump", filename, 1.0f);
	if (!entity.get())
	{
		glusLogPrint(GLUS_LOG_ERROR, "File not found %s", filename.c_str());
	...
	position = Point4(-50.0f, 10.0f, -16.0f);

	filename = "goober@walk.fbx";
	entity = entityFactory.loadFbxFile("GooberWalk", filename, 1.0f);
	if (!entity.get())
	{
		glusLogPrint(GLUS_LOG_ERROR, "File not found %s", filename.c_str());
	...
	position = Point4(-60.0f, 10.0f, -16.0f);

	filename = "Sadface.FBX";
	entity = entityFactory.loadFbxFile("Sadface", filename, 0.1f);
	if (!entity.get())
	{
		glusLogPrint(GLUS_LOG_ERROR, "File not found %s", filename.c_str());
	...
	position = Point4(-70.0f, 10.0f, -16.0f);

	filename = "Footbridge.fbx";
	entity = entityFactory.loadFbxFile("Footbridge", filename, 0.5f);
	if (!entity.get())
	{
		glusLogPrint(GLUS_LOG_ERROR, "File not found %s", filename.c_str());
	...
	entity->setPosition(position);
	entity->findInstanceNodeRecursive("Collision")->setVisible(false, true);

	ModelEntityManager::getInstance()->updateEntity(entity);

	// Mech

	position = Point4(-80.0f, 10.0f, -16.0f);

	filename = "mech_bot@walk_forward.FBX";
	entity = entityFactory.loadFbxFile("MechBot", filename, 0.05f);
	if (!entity.get())
	{
		glusLogPrint(GLUS_LOG_ERROR, "File not found %s", filename.c_str());
	...
	entity->setPosition(position);
	entity->setRotation(270.0f, 0.0f, 0.0f);
	entity->setAnimation(0, 0);
	...
	position = Point4(-90.0f, 10.0f, -16.0f);
	...
	position = Point4(-100.0f, 10.0f, -16.0f);
	...
	position = Point4(-110.0f, 10.0f, -16.0f);

	entity = entity->getNewInstance("Humanoid2");
	entity->setPosition(position);
	...
	position = Point4(-120.0f, 0.0f, -16.0f);
...
GLUSboolean updateGame(GLUSfloat deltaTime)
{
	...
	SkyManager::getInstance()->getSky("MySky")->render(CameraManager::getInstance()->getDefaultPerspectiveCamera()->getEye());
